...
class CookieManager(MutableMapping[str, str]):
    ...
    def __init__(self, *, path: str = None, prefix=""):
        ...
        self._prefix = prefix
        # Use a unique key per session
        self._component_key = st.session_state.setdefault("cookie_manager_key", f"CookieManager.sync_cookies.{uuid.uuid4()}")
        raw_cookie = self._run_component(save_only=False, key=self._component_key)
